/**
 * English translations / 英語翻訳
 */
export default {
  common: {
    dashboard: 'Dashboard',
    users: 'Users',
    settings: 'Settings',
    logout: 'Logout',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    create: 'Create',
    search: 'Search',
    loading: 'Loading...',
    noData: 'No data',
    confirm: 'Confirm',
    actions: 'Actions',
    success: 'Success',
    error: 'Error',
    id: 'ID',
    overview: 'Overview',
    comingSoon: 'coming soon.',
    yes: 'Yes',
    no: 'No',
    back: 'Back',
  },
  sidebar: {
    adminPanel: 'Admin Panel',
    userPanel: 'User Panel',
    overview: 'Overview',
    users: 'Users',
    employees: 'Employees',
    notebooklm: 'NotebookLM',
    notebooklmOperations: 'NotebookLM Operations',
    settings: 'Settings',
  },
  topbar: {
    lightMode: 'Switch to light mode',
    darkMode: 'Switch to dark mode',
    language: 'Language',
  },
  auth: {
    welcomeBack: 'Welcome Back',
    signInDescription: 'Sign in to your account',
    login: 'Login',
    loginTitle: 'Sign In',
    email: 'Email',
    emailPlaceholder: 'Enter your email',
    password: 'Password',
    passwordPlaceholder: 'Enter your password',
    loginButton: 'Sign In',
    loginFailed: 'Login failed. Please try again.',
    logout: 'Logout',
  },
  dashboard: {
    title: 'Overview',
    overview: 'Dashboard overview page',
  },
  users: {
    title: 'Users',
    createUser: 'Create User',
    editUser: 'Edit User',
    name: 'Name',
    namePlaceholder: 'Enter name',
    email: 'Email',
    emailPlaceholder: 'Enter email',
    role: 'Role',
    status: 'Status',
    createdAt: 'Created At',
    updatedAt: 'Updated At',
    noUsersFound: 'No users found.',
    deleteConfirm: 'Are you sure you want to delete this user?',
    deleteHeader: 'Confirm Deletion',
    deletedSuccess: 'User deleted successfully',
    deletedError: 'Failed to delete user',
    createdSuccess: 'User created successfully',
    createdError: 'Failed to create user',
    updatedSuccess: 'User updated successfully',
    updatedError: 'Failed to update user',
    emailInUse: 'This email is already in use',
    nameRequired: 'Name is required',
    nameMinLength: 'Name must be at least 2 characters',
    nameMaxLength: 'Name must be at most 50 characters',
    emailRequired: 'Email is required',
    emailInvalid: 'Invalid email format',
    emailChecking: 'Checking email...',
    emailAlreadyExists: 'This email is already taken',
    updateUser: 'Update User',
    activityLog: 'Activity Log',
    noActivity: 'No activity recorded yet.',
    allRoles: 'All Roles',
    allStatuses: 'All Statuses',
    searchPlaceholder: 'Search by name or email...',
    dateRange: 'Date Range',
    datePlaceholder: 'Select date range',
    clearFilters: 'Clear Filters',
    note: 'Note',
    notePlaceholder: 'Enter a note (optional)',
    noteMaxLength: 'Note must be at most 500 characters',
    birthday: 'Birthday',
    birthdayPlaceholder: 'Select birthday',
    lastLogin: 'Last Login',
    points: 'Points',
    roles: {
      admin: 'Admin',
      user: 'User',
      moderator: 'Moderator',
    },
    statuses: {
      active: 'Active',
      inactive: 'Inactive',
      suspended: 'Suspended',
    },
  },
  settings: {
    title: 'Settings',
    overview: 'Settings page',
  },
  profile: {
    title: 'My Profile',
    myProfile: 'My Profile',
    editProfile: 'Edit Profile',
    name: 'Full Name',
    email: 'Email',
    role: 'Role',
    birthday: 'Birthday',
    birthdayPlaceholder: 'Select birthday',
    note: 'Note',
    notePlaceholder: 'Enter a note (optional)',
    avatar: 'Avatar',
    avatarTooBig: 'Image is too large. Please select an image under 2MB.',
    saveChanges: 'Save Changes',
    savedSuccess: 'Profile updated successfully',
    changePassword: 'Change Password',
    currentPassword: 'Current Password',
    newPassword: 'New Password',
    confirmPassword: 'Confirm New Password',
    passwordChanged: 'Password changed successfully',
    passwordMismatch: 'Passwords do not match',
    currentPasswordWrong: 'Current password is incorrect',
    passwordMinLength: 'Password must be at least 8 characters',
    nameMinLength: 'Name must be at least 2 characters',
    nameMaxLength: 'Name must be at most 100 characters',
    noteMaxLength: 'Note must be at most 500 characters',
  },
  employees: {
    title: 'Employees',
    createEmployee: 'Create Employee',
    editEmployee: 'Edit Employee',
    employeeCode: 'Employee Code',
    fullName: 'Full Name',
    email: 'Email',
    phone: 'Phone',
    department: 'Department',
    position: 'Position',
    hireDate: 'Hire Date',
    salary: 'Salary',
    status: 'Status',
    createdAt: 'Created At',
    updatedAt: 'Updated At',
    noEmployeesFound: 'No employees found.',
    deleteConfirm: 'Are you sure you want to delete this employee?',
    deleteHeader: 'Confirm Deletion',
    deletedSuccess: 'Employee deleted successfully',
    deletedError: 'Failed to delete employee',
    createdSuccess: 'Employee created successfully',
    createdError: 'Failed to create employee',
    updatedSuccess: 'Employee updated successfully',
    updatedError: 'Failed to update employee',
    allDepartments: 'All Departments',
    allStatuses: 'All Statuses',
    searchPlaceholder: 'Search by name, email or code…',
    clearFilters: 'Clear Filters',
    employeeCodeRequired: 'Employee code is required',
    employeeCodeInvalid: 'Employee code must be 3–10 alphanumeric characters',
    fullNameRequired: 'Full name is required',
    fullNameMinLength: 'Full name must be at least 2 characters',
    fullNameMaxLength: 'Full name must be at most 100 characters',
    emailRequired: 'Email is required',
    emailInvalid: 'Invalid email format',
    salaryMin: 'Salary must be at least 0',
    hireDateRequired: 'Hire date is required',
    hireDateFuture: 'Hire date cannot be in the future',
    departments: {
      engineering: 'Engineering',
      hr: 'HR',
      finance: 'Finance',
      marketing: 'Marketing',
      operations: 'Operations',
    },
    positions: {
      engineer: 'Engineer',
      senior_engineer: 'Senior Engineer',
      team_lead: 'Team Lead',
      manager: 'Manager',
      director: 'Director',
      analyst: 'Analyst',
      specialist: 'Specialist',
      intern: 'Intern',
    },
    statuses: {
      active: 'Active',
      inactive: 'Inactive',
    },
  },
  notebooklmOperations: {
    title: 'NotebookLM Operations',
    monitorTitle: 'Job Monitor',
    retryTitle: 'Create Retry Request',
    dlqEditTitle: 'Edit DLQ Item',
    filterFailedOnly: 'Only failed/dead-letter jobs',
    retrySaved: 'Retry request submitted',
    dlqSaved: 'DLQ note updated',
    purgeConfirm: 'Purge selected dead-letter item permanently?',
  },
  notebooklmWorkspace: {
    role: 'Role',
    allRoles: 'All Roles',
    rowsPerPage: 'Rows per page',
    searchPlaceholder: 'Search workspace by name...',
    clearFilters: 'Clear Filters',
    deleteConfirm: 'Are you sure you want to delete this workspace? This action cannot be undone.',
    deleteHeader: 'Confirm Workspace Deletion',
    deletedSuccess: 'Workspace deleted successfully',
    deletedError: 'Failed to delete workspace',
    openChat: 'Open Chat',
    memberManager: {
      title: 'Workspace Members',
      searchPlaceholder: 'Search users by name or email...',
      searchResultTitle: 'Search results',
      noCandidates: 'No matching users found.',
      selectRole: 'Select role',
      roleRequired: 'Role selection is required.',
      addOrUpdate: 'Add or update member',
      noMembers: 'No members in this workspace yet.',
      readonlyHint: 'Only workspace owners can add or update members.',
      roles: {
        owner: 'owner',
        editor: 'editor',
        viewer: 'viewer',
      },
    },
  },
  notebooklmChat: {
    title: 'Chat Sessions',
    newSession: 'New Session',
    openSession: 'Open',
    sessionTitle: 'Session Title',
    titlePlaceholder: 'Enter session title (optional)',
    createSession: 'Create Session',
    sendMessage: 'Send',
    messagePlaceholder: 'Type your question...',
    sources: 'Sources',
    noMessages: 'No messages yet. Ask a question to get started.',
    noSessions: 'No chat sessions yet.',
    deletedSuccess: 'Session deleted successfully',
    deletedError: 'Failed to delete session',
    createdSuccess: 'Session created successfully',
    updatedSuccess: 'Session updated successfully',
    llmProvider: 'LLM Provider',
    llmProviderOllama: 'Ollama (Local)',
    llmProviderMock: 'Mock Server',
    llmProviderGemini: 'Gemini (Google)',
    provider: 'Provider',
    thinking: 'Thinking...',
  },
  vocabularies: {
    title: 'Vocabulary Management',
    createTitle: 'Create Vocabulary',
    editTitle: 'Edit Vocabulary',
    createNew: 'Create Vocabulary',
    searchPlaceholder: 'Search by kanji, meaning, romaji...',
    filterLevel: 'All Levels',
    filterStatus: 'All Statuses',
    filterTag: 'Filter by tag',
    col: {
      kanji: 'Kanji',
      meaningVi: 'Vietnamese Meaning',
      level: 'Level',
      status: 'Status',
      tags: 'Tags',
      createdAt: 'Created At',
    },
    deleteConfirm: 'Are you sure you want to delete this vocabulary?',
    deleteHeader: 'Confirm Deletion',
    deletedSuccess: 'Vocabulary deleted successfully',
    deletedError: 'Failed to delete vocabulary',
    createdSuccess: 'Vocabulary created successfully',
    createdError: 'Failed to create vocabulary',
    updatedSuccess: 'Vocabulary updated successfully',
    updatedError: 'Failed to update vocabulary',
    form: {
      meaningVi: 'Vietnamese Meaning',
      meaningViPlaceholder: 'Enter Vietnamese meaning',
      meaningViRequired: 'Vietnamese meaning is required',
      meaningViMax: 'Vietnamese meaning must be at most 500 characters',
      hiragana: 'Hiragana / Katakana',
      hiraganaPlaceholder: 'Enter hiragana/katakana',
      romaji: 'Romaji',
      romajiPlaceholder: 'Enter romaji',
      kanji: 'Kanji',
      kanjiPlaceholder: 'Enter kanji',
      sinoVi: 'Sino-Vietnamese',
      sinoViPlaceholder: 'Enter sino-vietnamese',
      level: 'Level',
      levelRequired: 'Level is required',
      mediaUrl: 'Image URL',
      mediaUrlPlaceholder: 'https://cdn.example.com/image.jpg',
      mediaUrlInvalid: 'Must be a valid URL',
      note: 'Notes',
      notePlaceholder: 'Enter notes (optional)',
      tags: 'Tags',
      tagPlaceholder: 'Type to add tags',
      relatedWords: 'Related Words',
      relatedPlaceholder: 'Search related words',
      synonyms: 'Synonyms',
      synonymPlaceholder: 'Search synonyms',
      antonyms: 'Antonyms',
      antonymPlaceholder: 'Search antonyms',
      status: 'Status',
      statusRequired: 'Status is required',
      tabInfo: 'Information',
      tabAudit: 'Audit',
      tabAnalytics: 'Analytics',
      cancelDirtyConfirm: 'You have unsaved changes. Are you sure you want to leave?',
      cancelDirtyHeader: 'Unsaved Changes',
      discardChanges: 'Discard Changes',
    },
    audit: {
      createdBy: 'Created By',
      updatedBy: 'Last Updated By',
      version: 'Version',
      createdAt: 'Created At',
      updatedAt: 'Updated At',
      changeLog: 'Change History',
      reports: 'User Reports',
      resolve: 'Mark Resolved',
      pending: 'Mark Pending',
    },
    analytics: {
      learnCount: 'Learn Count',
      favoriteCount: 'Favorite Count',
    },
  },
  learn: {
    level: {
      pageTitle: 'Choose Study Level',
      pageSubtitle: 'Choose your JLPT level to start studying',
      words: 'words',
      known: '✓ Known',
      learning: '↻ Learning',
      newCount: '✦ New',
      studyButton: 'Study {level}',
    },
    vocabList: {
      pageTitle: '{level} Vocabulary List',
      backToLevel: 'Back to Level Selection',
      filterAll: 'All',
      filterNew: 'New',
      filterLearning: 'Learning',
      filterKnown: 'Known',
      studyAll: 'Study All',
      studyUnknown: 'Study Unknown',
      colKanji: 'Kanji',
      colHiragana: 'Hiragana',
      colRomaji: 'Romaji',
      colMeaning: 'Meaning',
      colProgress: 'Progress',
      colFavorite: 'Favorite',
      statusNew: 'New',
      statusLearning: 'Learning',
      statusKnown: 'Known',
    },
    session: {
      pageTitle: '{level} Flashcard',
      configAlt: 'Card settings',
      noVocabs: 'No unlearned words at this level.',
      backToList: 'Back to List',
      quitConfirm: 'Are you sure you want to quit the session?',
      quit: 'Quit',
      markKnown: 'Known',
      markUnknown: 'Review',
      saveSuccess: 'Progress saved',
      saveDetail: 'Progress has been saved successfully',
      complete: 'Session Complete 🎉',
      knownLabel: '✓ Correct / Known',
      unknownLabel: '✗ Need Review',
      favoriteLabel: '★ Favorites',
      accuracy: 'Accuracy:',
      retry: 'Retry',
      backToListSummary: 'Back to List',
    },
    card: {
      tapToFlip: 'Tap to flip',
    },
    config: {
      title: 'Card Settings',
      flipDirection: 'Flip Direction',
      kanjiToMeaning: 'Kanji → Meaning',
      meaningToKanji: 'Meaning → Kanji',
      frontFields: 'Front Fields',
      backFields: 'Back Fields',
      showKanji: 'Show Kanji',
      hideKanji: 'Hide Kanji',
      showHiragana: 'Show Hiragana',
      hideHiragana: 'Hide Hiragana',
      showRomaji: 'Show Romaji',
      hideRomaji: 'Hide Romaji',
      showMeaning: 'Show Meaning',
      hideMeaning: 'Hide Meaning',
    },
  },
} as const;
