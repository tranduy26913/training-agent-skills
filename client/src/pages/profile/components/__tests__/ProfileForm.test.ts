...
describe('ProfileForm', () => {
  ...
  it('emits submit with correct dto on form submit', async () => {
    ...
    // Name is 'Alice'  Evalid, so submit should emit.
    // Allow async validation to complete.
    await new Promise((r) => setTimeout(r, 50));
    const submitted = wrapper.emitted('submit');
    if (submitted && submitted.length > 0) {
      const dto = submitted[0][0] as { name: string };
  ...
  it('formats birthday using local date (not UTC) to avoid off-by-one day across timezones', async () => {
    // e.g. UTC+7: new Date('2026-04-23') is 2026-04-22T17:00:00Z  EtoISOString() would return '2026-04-22', not '2026-04-23'
    ...
    const submitted = wrapper.emitted('submit');
    if (submitted && submitted.length > 0) {
      const dto = submitted[0][0] as { birthday?: string };
