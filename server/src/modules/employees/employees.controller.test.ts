...
// CobolGateway のモックオブジェクトを作成 / Create a plain mock object for CobolGateway
...
// モックゲートウェイを注入して Express アプリを構築 / Build app with injected mock gateway
...
describe('EmployeesController', () => {
  ...
  describe('GET /api/employees', () => {
    it('returns 200 with paginated data on success', async () => {
      ...
      const res = await request(buildApp(gateway)).get('/api/employees');
  ...
  describe('GET /api/employees/:id', () => {
    it('returns 200 with employee data', async () => {
      ...
      const res = await request(buildApp(gateway)).get('/api/employees/1');
    ...
    it('returns 404 when employee not found', async () => {
      // 従業員が見つからない場合に 404 を返す
      gateway.getEmployee.mockRejectedValueOnce(
        new ServiceError('Employee not found', 404),
      );

      const res = await request(buildApp(gateway)).get('/api/employees/999');
  ...
  describe('DELETE /api/employees/:id', () => {
    it('returns 200 on successful delete', async () => {
      ...
      const res = await request(buildApp(gateway)).delete('/api/employees/1');
    ...
    it('returns 404 when employee not found on delete', async () => {
      // 削除対象の従業員が見つからない場合に 404 を返す
      gateway.deleteEmployee.mockRejectedValueOnce(
        new ServiceError('Employee not found', 404),
      );

      const res = await request(buildApp(gateway)).delete('/api/employees/999');
