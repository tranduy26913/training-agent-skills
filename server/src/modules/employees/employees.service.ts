...
export class CobolGateway {
  ...
  constructor() {
    // COBOLサービスのベースURL / Base URL for COBOL CGI server
    this.baseUrl = process.env.COBOL_CGI_URL ?? 'http://localhost:8081/cgi-bin';
  }

  // COBOLエラーコードをHTTPステータスにマップ / Map COBOL error code to HTTP status
  ...
  // COBOLレスポンスを解析 / Parse and validate COBOL JSON response
  ...
  // 従業員一覧取得 / Get paginated list of employees
  ...
  // 従業員詳細取得 / Get single employee by ID
